...
# maybe refactor it so its not being transformed with every pipe
  ...
  def move(self):
    ...
    self.current_y_displacement = (-1 if math.sin((self.x - self.gap_height)/ 100) > 0 else 1) * 2
  ...
  def is_colliding(self, bird):
    ...
    bottom_pipe_offset = (self.x - bird.x, self.bottom_pipe_y - round(bird.y))
    top_pipe_offset = (self.x - bird.x, self.top_pipe_y - round(bird.y))

    bottom_pipe_point_of_collision = bird_mask.overlap(bottom_pipe_mask, bottom_pipe_offset)
    top_pipe_point_of_collision = bird_mask.overlap(top_pipe_mask, top_pipe_offset)
